...
describe('la bibliothèque de profils', () => {
  it('part vide, sans que ce soit une anomalie', async () => {
    ...
    expect(await listerProfils(base, COMPTE)).toEqual([])
  })

  it('rend le profil déposé, tel qu’il a été déposé', async () => {
    await ecrireProfil(base, COMPTE, 'mon-v8.json', UN_PROFIL)

    expect(JSON.parse((await lireProfil(base, COMPTE, 'mon-v8.json')) ?? '')).toEqual(
  ...
  it('liste au format que le cœur attend', async () => {
    ...
    const [entree] = await listerProfils(base, COMPTE)
  ...
  it('avance la date quand on redépose', async () => {
    ...
    const [avant] = await listerProfils(base, COMPTE)

    await ecrireProfil(base, COMPTE, 'mon-v8.json', UN_PROFIL)
    const [apres] = await listerProfils(base, COMPTE)
  ...
  it('rend le nom de fichier déposé, et non un nom dérivé du profil', async () => {
    ...
    await ecrireProfil(base, COMPTE, 'autre-chose.json', UN_PROFIL)

    expect(nomsDe(await listerProfils(base, COMPTE))).toEqual(['autre-chose.json'])
  })
  ...
  it('remplace quand on redépose le même nom', async () => {
    ...
    const liste = await listerProfils(base, COMPTE)
  ...
  it('garde deux profils sous deux noms', async () => {
    await ecrireProfil(base, COMPTE, 'un.json', UN_PROFIL)
    await ecrireProfil(base, COMPTE, 'deux.json', UN_PROFIL)

    expect(nomsDe(await listerProfils(base, COMPTE))).toEqual(['deux.json', 'un.json'])
  })

  it('rend rien pour un profil qu’on n’a pas déposé', async () => {
    expect(await lireProfil(base, COMPTE, 'jamais-vu.json')).toBeNull()
  })
  ...
  it('refuse ce qui n’est pas du JSON, plutôt que de le ranger', async () => {
    ...
    expect(await ecrireProfil(base, COMPTE, 'bancal.json', 'ceci{ n’est pas')).toBe(
      'illisible',
    )
    expect(await listerProfils(base, COMPTE)).toEqual([])
  })
  ...
  it('ne valide pas la forme du profil, et c’est voulu', async () => {
    ...
    // déjà payé ailleurs.
    expect(await ecrireProfil(base, COMPTE, 'etrange.json', '{"rien":"de connu"}')).toBe(
      'écrit',
    )
  })
})

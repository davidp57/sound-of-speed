...
| [DURCIR](DURCIR/spec.md) — sécuriser le code et les données : fermer le service des échantillons, borner ce qu'un compte dépense, et prouver ce qu'un compte peut atteindre de ce qu'un autre a déposé | 🔄 10/14 ✅ + 3 🧑 — les échantillons demandent un compte, une banque peut être réservée, l'isolation est prouvée route par route, un nom déposé ne compose plus de chemin, le serveur sert une politique de contenu et relève sa dépense chaque jour ; le chiffrement au repos et les identifiants opaques sont tranchés. Reste la borne de volume, qui attend les chiffres |
| [REGIE](REGIE/spec.md) — administrer les comptes depuis un écran : rôle d'administrateur, attribution des rôles et des banques, plafond de volume, effacement, et lecture des données d'un compte quand il l'autorise | ⬜ 0/9 — instruit, spécifié et découpé le 15 septembre |

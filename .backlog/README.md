...
| [BANQUES](BANQUES/spec.md) — accueillir plusieurs banques de son : découverte, mesure outillée, choix par profil | 🧑 6/6 — les deux tickets du 17 septembre sont livrés : une banque absente se nomme au lieu de rendre un nom de fichier, et les quatre banques livrées sonnent au même niveau — le six en ligne sortait 5,7 dB sous le V8, mesuré à travers le mixage, et ses gains sont remontés d'autant ; reste l'oreille |
...
| [REMONTEE](REMONTEE/spec.md) — tout ce qui naît dans la voiture remonte tout seul : traces, journal, relevés de mesure, profils, sur un accord unique | 🧑 — le sens montant est prouvé, et le **retour** a son geste depuis le 17 septembre — « Reprendre les profils du serveur » remplace au lieu de dupliquer ([ticket 08](REMONTEE/tickets/08-reprendre-du-serveur-sans-dupliquer.md)) ; reste l'essai contre le NAS |
...
| [ETAGEMENT](ETAGEMENT/spec.md) — une boîte à sept rapports, étagée pour tenir bas : 50 en quatrième, 80 en cinquième, 110 en sixième, et les passages plus tôt en mode Route | 🧑 les trois premiers tickets sont livrés ; reste l'écoute en roulant, et la question de l'étagement par moteur (ticket 04) |
| [REMISE-A-ZERO](REMISE-A-ZERO/spec.md) — tout remettre comme au premier jour : David demande le 17 septembre **deux** boutons plutôt qu'un, reprendre les profils et vider la couche d'ajustements | ⬜ le périmètre s'est précisé, une question reste : profils livrés ou profils du serveur ? |
...
| [DURCIR](DURCIR/spec.md) — sécuriser le code et les données : fermer le service des échantillons, borner ce qu'un compte dépense, et prouver ce qu'un compte peut atteindre de ce qu'un autre a déposé | 🔄 11/17 ✅ + 3 🧑 — les échantillons demandent un compte, une banque peut être réservée, l'isolation est prouvée route par route, un nom déposé ne compose plus de chemin, le serveur sert une politique de contenu, et son relevé de dépense se lit enfin — écrit en base et affiché dans la régie, là où le journal du conteneur partait avec le conteneur ; il ne reste qu'à laisser tourner et lire ; le chiffrement au repos et les identifiants opaques sont tranchés. Reste la place à faire quand le plafond est atteint : instruite et découpée le 15 septembre en trois tickets (07 à 09) |
...
| [ESSAI-16](ESSAI-16/spec.md) — dix points de la sortie du 16 septembre : un rétrogradage forcé qui ne partait jamais, des bandeaux qui recouvrent les commandes, un suivi GPS qui se relance sans cesse | 🔄 7/14 — la régie donne l'heure locale, et le volume est **mesuré** : à fond, un échantillon sur onze est rogné et la distorsion atteint −18 dB, donc la demande d'un curseur plus haut ne peut pas être satisfaite telle quelle ; trois voies chiffrées attendent David. Le réarmement du rétrogradage (07) attend les traces du 16 |
| [INTERFACE](INTERFACE/spec.md) — refaire l'écran pour la taille qu'il a vraiment : 773 × 575 px utiles dans la voiture, et non la fenêtre d'un poste de travail. Absorbe TABLEAU-DE-BORD et UI-DEFILEMENT | 🔄 1/1 livré — le journal porte les quatre cadres et les reprend à chaque changement, et une mire à la carte bancaire donne le millimètre par pixel ; reste à ouvrir la mire une fois et à rouler. David a tranché la colonne du milieu : le rapport au-dessus du sélecteur, les deux entre les compteurs |

...
| [DURCIR](DURCIR/spec.md) — sécuriser le code et les données : fermer le service des échantillons, borner ce qu'un compte dépense, et prouver ce qu'un compte peut atteindre de ce qu'un autre a déposé | 🔄 8/14 ✅ + 2 🧑 — les échantillons demandent un compte et une banque peut être réservée, l'isolation entre comptes est prouvée route par route, un nom déposé ne compose plus de chemin, le serveur sert une politique de contenu, la copie des rôles porte son compte et un seul code de liaison reste vivant ; restent quatre tickets qui demandent le NAS (mesurer, borner, identifiants opaques, verdict sur les données en clair) |
| [REGIE](REGIE/spec.md) — administrer les comptes depuis un écran : rôle d'administrateur, attribution des rôles, des banques et des quotas | ⬜ cadre posé le 15 septembre, pas découpé |

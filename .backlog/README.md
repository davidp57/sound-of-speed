...
| [DURCIR](DURCIR/spec.md) — sécuriser le code et les données : fermer le service des échantillons, borner ce qu'un compte dépense, et prouver ce qu'un compte peut atteindre de ce qu'un autre a déposé | 🔄 9/14 ✅ + 3 🧑 — les échantillons demandent un compte, une banque peut être réservée, l'isolation est prouvée route par route, un nom déposé ne compose plus de chemin, le serveur sert une politique de contenu et relève sa dépense chaque jour ; le chiffrement au repos est tranché — on ne le fait pas. Restent les identifiants opaques, et la borne de volume qui attend les chiffres |

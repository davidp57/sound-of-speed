...
**Statut :** 🧑 attend David — les huit tickets sont livrés ; il reste l'écoute du
rejeu sonore, qui demande une capture réelle

## Ce que la revue des critères a donné, le 12 septembre 2026

Les quarante-six critères des sept premiers tickets ont été repris un par un
contre le code et les tests. Quarante-deux étaient établis sans que personne les
ait cochés. Les quatre autres ont été traités le jour même :

1. **Le trou d'un signal perdu se voit déjà**, contrairement à ce qu'une première
   lecture avait conclu : une relance du suivi est inscrite dans le journal, le
   relecteur lit le journal comme la capture et la marque sur sa timeline, et là
   où la source n'a rien émis la valeur s'affiche comme interpolée. Rien à écrire.
2. **Le code mort est parti** : des règles de style que plus aucun élément ne
   portait, la lecture de traces en fichier que seul son propre test appelait, et
   deux commentaires qui décrivaient encore le geste supprimé.
3. **La liste affiche la durée** de chaque trajet, en borne basse lue sur le
   nombre de tranches — choisir quel trajet relire ne demande plus de charger
   celui qu'on est en train de choisir.
4. **Le README disait faux** : il décrivait encore l'export et la réimportation
   des traces en fichier, et leur enregistrement depuis l'écran Télémétrie —
   trois gestes retirés par le ticket 04.

...
- [x] Le workflow d'intégration lance `typecheck`, `lint`, `test` et `build` sur
      chaque PR vers `develop` et vers `main`
- [x] Le contrôle qualité de `CLAUDE.md` n'a plus de version « aujourd'hui » et
      de version « cible » : il n'y en a plus qu'une, celle des quatre commandes
- [x] La couverture de `core/` est mesurée et rapportée, avec le chiffre atteint
      inscrit dans la spécification du lot
- [x] Le tableau « État du projet » du README porte le lot et son état
- [x] La checklist par changement de `CLAUDE.md` ne mentionne plus les tests
      comme conditionnels
